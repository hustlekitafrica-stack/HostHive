...
export default function ReportsPage() {
  ...
  const handlePrint = () => {
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      <div class="brand">Host Hive</div>
      <h1>${reportData.title}</h1>
      <div class="period">Period: ${reportData.period}</div>
      <div class="sg">${summaryHTML}</div>
      <table><thead><tr>${headHTML}</tr></thead><tbody>${bodyHTML}</tbody></table>
      <div class="footer">Generated by Host Hive &mdash; ${new Date().toLocaleDateString('en-KE',{dateStyle:'full'})}</div>
    </body></html>`;
  ...
  const handleEmailShare = () => {
    if (!reportData) return;
    const subject = encodeURIComponent(`${reportData.title} — Host Hive`);
    const lines = reportData.summary.map(s => `${s.label}: ${s.value}`).join('\n');
    const body = encodeURIComponent(
      `${reportData.title}\nPeriod: ${reportData.period}\n\n${lines}\n\nGenerated by Host Hive`
    );
                ...
                {loadingReport ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-400 mt-4 text-right">
                      Generated by Host Hive &mdash; {new Date().toLocaleDateString('en-KE', { dateStyle: 'full' })}
                    </p>
                  </>
                ) : (
