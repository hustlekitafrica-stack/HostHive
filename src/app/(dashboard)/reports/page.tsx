...
export default function ReportsPage() {
  ...
  const handlePrint = () => {
    ...
    <style>
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      <div class="brand">HostBooks KE</div>
      <h1>${reportData.title}</h1>
      <div class="period">Period: ${reportData.period}</div>
      <div class="sg">${summaryHTML}</div>
      <table><thead><tr>${headHTML}</tr></thead><tbody>${bodyHTML}</tbody></table>
      <div class="footer">Generated by HostBooks KE &mdash; ${new Date().toLocaleDateString('en-KE',{dateStyle:'full'})}</div>
    </body></html>`;
  ...
  const handleEmailShare = () => {
    if (!reportData) return;
    const subject = encodeURIComponent(`${reportData.title} — HostBooks KE`);
    const lines = reportData.summary.map(s => `${s.label}: ${s.value}`).join('\n');
    const body = encodeURIComponent(
      `${reportData.title}\nPeriod: ${reportData.period}\n\n${lines}\n\nGenerated by HostBooks KE`
    );
            ...
            <div className="bg-white rounded-2xl w-full max-w-5xl shadow-2xl flex flex-col">
              ...
              <div className="flex flex-wrap items-center gap-3 px-6 py-4 border-b border-gray-200">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={handleEmailShare} disabled={!reportData}
                  ...
                  <button onClick={handleWhatsAppShare} disabled={!reportData}
                  ...
                  <button onClick={handleNativeShare} disabled={!reportData}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-xs font-medium hover:bg-gray-200 disabled:opacity-40 transition-colors">
                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                ...
                {loadingReport ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-400 mt-4 text-right">
                      Generated by HostBooks KE &mdash; {new Date().toLocaleDateString('en-KE', { dateStyle: 'full' })}
                    </p>
                  </>
                ) : (
