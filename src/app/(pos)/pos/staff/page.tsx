...
interface StaffForm { name: string; role: Role; pin: string }
...
function StaffFormFields({
  form, setForm, pinLabel = 'PIN (4 digits)', pinRequired = true,
}: { form: StaffForm; setForm: React.Dispatch<React.SetStateAction<StaffForm>>; pinLabel?: string; pinRequired?: boolean }) {
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <input type="password" inputMode="numeric" maxLength={4} pattern="\d{4}" placeholder="••••" value={form.pin}
          onChange={e => { const val = e.target.value.replace(/\D/g, '').slice(0, 4); setForm(f => ({ ...f, pin: val })); }}
          className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2.5 text-white text-sm tracking-widest font-mono placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500" />
...
export default function PosStaffPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setShowAddModal(false); setAddForm(EMPTY_FORM); loadStaff();
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      const payload: Record<string, unknown> = { name: editForm.name.trim(), role: editForm.role };
      ...
      setShowEditModal(false); setEditStaff(null); loadStaff();
    } catch { toast.error('Network error'); } finally { setSaving(false); }
  };
  ...
  const toggleActive = async (member: StaffMember) => {
    ...
    try {
      ...
      setStaff(prev => prev.map(s => s.id === member.id ? { ...s, active: newActive } : s));
    } catch { toast.error('Network error'); } finally { setTogglingId(null); }
  };

  const fmtDate = (iso: string) => new Date(iso).toLocaleDateString('en-KE', { dateStyle: 'medium' });
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-indigo-500/20 rounded-xl"><Users className="w-5 h-5 text-indigo-400" /></div>
            <div>
              <h1 className="text-xl font-bold">POS Staff Management</h1>
              <p className="text-xs text-slate-400">{activeCount} active · {inactiveCount} inactive</p>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-700/60">
                  {staff.map(member => (
                    <tr key={member.id} className={`transition-colors hover:bg-slate-700/30 ${!member.active ? 'opacity-60' : ''}`}>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button onClick={() => toggleActive(member)} disabled={togglingId === member.id}
                            ...
                            className={`p-1.5 rounded-lg transition-colors ${member.active ? 'text-slate-400 hover:text-red-400 hover:bg-red-400/10' : 'text-slate-400 hover:text-green-400 hover:bg-green-400/10'} disabled:opacity-40`}>
                            {togglingId === member.id ? <Loader2 className="w-4 h-4 animate-spin" />
                              : member.active ? <ToggleRight className="w-5 h-5 text-green-400" />
          ...
          <Modal title="Add Staff Member" onClose={() => setShowAddModal(false)}>
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-2 bg-slate-700/40 rounded-lg px-3 py-2.5">
                ...
                <p className="text-xs text-slate-400">The PIN is used to authenticate this staff member at the POS terminal. Store it securely.</p>
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleAdd} disabled={saving} className="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 disabled:text-slate-500 text-sm font-semibold transition-colors flex items-center justify-center gap-2">
        ...
        {showEditModal && editStaff && (
          <Modal title={`Edit: ${editStaff.name}`} onClose={() => setShowEditModal(false)}>
            <div className="space-y-4">
              <StaffFormFields form={editForm} setForm={setEditForm} pinLabel="New PIN (4 digits)" pinRequired={false} />
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleEdit} disabled={saving} className="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 disabled:text-slate-500 text-sm font-semibold transition-colors flex items-center justify-center gap-2">
