...
export default function PosReportsPage() {
  ...
  useEffect(() => {
    fetch('/api/pos/staff').then(r => r.json()).then(d => setStaffList(d.staff ?? [])).catch(() => {});
  ...
  const exportCsv = () => {
    ...
    const header = ['Order #', 'Room/Location', 'Staff', 'Total', 'Payment Method', 'Paid At'];
    ...
    a.href = url; a.download = `pos-orders-${dateFrom}-to-${dateTo}.csv`;
    a.click();
          ...
          <div className="flex flex-wrap items-end gap-4">
            <div>
              ...
              <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
                className="bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50" />
            </div>
            <div>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
              <KpiCard label="Total Revenue" value={fmtShort(summary?.total_revenue ?? 0, currency)}
                sub={fmt(summary?.total_revenue ?? 0, currency)}
                icon={<TrendingUp className="w-5 h-5 text-green-400" />} color="bg-green-500/20" />
              <KpiCard label="Total Orders" value={String(summary?.total_orders ?? 0)}
                icon={<ShoppingBag className="w-5 h-5 text-blue-400" />} color="bg-blue-500/20" />
              <KpiCard label="Avg Order Value" value={fmtShort(summary?.avg_order_value ?? 0, currency)}
                icon={<DollarSign className="w-5 h-5 text-purple-400" />} color="bg-purple-500/20" />
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-2xl p-5 space-y-3">
                ...
                <div className="space-y-2">
                  {[
                    { icon: <Banknote className="w-3.5 h-3.5 text-green-400" />,   label: 'Cash',  val: summary?.cash_total  ?? 0 },
                    { icon: <Smartphone className="w-3.5 h-3.5 text-emerald-400" />, label: 'M-Pesa', val: summary?.mpesa_total ?? 0 },
                    { icon: <CreditCard className="w-3.5 h-3.5 text-blue-400" />,  label: 'Card',  val: summary?.card_total  ?? 0 },
                  ].map(row => (
                    <div key={row.label} className="flex items-center justify-between text-sm">
                      <div className="flex items-center gap-1.5 text-slate-300">{row.icon}{row.label}</div>
                      <span className="font-semibold tabular-nums">{fmtShort(row.val, currency)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-700"><h2 className="font-semibold text-slate-200">Top 10 Items</h2></div>
              {topItems.length === 0 ? (
                  ...
                  <tbody className="divide-y divide-slate-700/60">
                    {topItems.map((item, i) => (
                      <tr key={item.name} className="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="px-5 py-3 text-right tabular-nums text-green-400 font-semibold">{fmt(item.total_revenue, currency)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {orders.length === 0 ? (
                ...
                <>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-slate-800/80 text-slate-400 text-xs uppercase tracking-wider">
                          ...
                          <th className="px-5 py-3 text-left font-medium">Room/Location</th>
                      ...
                      <tbody className="divide-y divide-slate-700/60">
                        {pageOrders.map(o => (
                          <tr key={o.id} className="hover:bg-slate-700/30 transition-colors">
                            <td className="px-5 py-3 font-mono text-xs text-slate-400">{o.order_number ?? `#${o.id.slice(0, 8)}`}</td>
                            <td className="px-5 py-3 text-slate-300">{o.table_name ?? o.table_id ?? '—'}</td>
                            <td className="px-5 py-3 text-slate-300">{o.staff_name ?? '—'}</td>
                            <td className="px-5 py-3 text-right font-semibold tabular-nums text-white">{fmt(o.total, currency)}</td>
                            <td className="px-5 py-3 text-center">{methodBadge(o.payment_method)}</td>
                            <td className="px-5 py-3 text-xs text-slate-400">{o.paid_at ? fmtDateTime(o.paid_at) : '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div className="flex items-center justify-between px-5 py-3 border-t border-slate-700">
                    <p className="text-xs text-slate-500">Page {page + 1} of {totalPages} — {orders.length} orders</p>
