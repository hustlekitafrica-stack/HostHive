...
interface Props {
  initialProperties: StayProperty[];
...
export default function RoomsListClient({
  initialProperties, initialCheckIn, initialCheckOut, initialGuests, initialRooms,
}: Props) {
  ...
  const [properties,  setProperties]  = useState<StayProperty[]>(initialProperties);
  ...
  const [isMultiMode,  setIsMultiMode]  = useState(initialRooms >= 2);
  const [cart,         setCart]         = useState<{ property: StayProperty; qty: number }[]>([]);
    ...
    <div className="min-h-screen bg-[#f8fafc]">
      ...
      <div className="hidden md:block px-4 sm:px-6" style={{ background: '#1e293b' }}>
        <div className="pt-16 pb-6 px-2 max-w-5xl mx-auto">
          <SearchWidget initialCheckIn={checkIn} initialCheckOut={checkOut} initialAdults={adults} initialRooms={rooms} />
        </div>
      </div>

      {/* MOBILE: small dark strip */}
          ...
          <button onClick={() => setSearchExpanded(true)}
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">
                {checkIn && checkOut ? `${fmt(checkIn)} – ${fmt(checkOut)}${nights > 0 ? ` (${nights} night${nights !== 1 ? 's' : ''})` : ''} · ` : 'Any dates · '}
          ...
          <aside className="hidden md:block w-64 flex-shrink-0 sticky top-20 self-start max-h-[calc(100vh-5.5rem)] overflow-y-auto rounded-2xl">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="mb-5">
                ...
                {[['price_asc','Price: Low → High'],['price_desc','Price: High → Low'],['name','Name A–Z']].map(([v,l]) => (
                  <label key={v} className="flex items-center gap-2.5 py-1.5 cursor-pointer">
                    <input type="radio" name="sort" checked={sortBy === v} onChange={() => setSortBy(v as 'price_asc'|'price_desc'|'name')} className="accent-green-600" />
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                <a href="https://maps.google.com/maps?q=Kogelo+Suites,Kogelo,Siaya,Kenya" target="_blank" rel="noopener noreferrer"
                  ...
                  <Map className="w-4 h-4 flex-shrink-0" /> View on Map
                </a>
              </div>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => { setIsMultiMode(v => !v); setCart([]); }} aria-label="Toggle group booking"
                  className={`relative inline-flex h-7 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 focus:outline-none ${isMultiMode ? 'bg-green-500' : 'bg-gray-300'}`}>
                  <span className={`inline-block h-6 w-6 transform rounded-full bg-white shadow-md transition-transform duration-200 ${isMultiMode ? 'translate-x-5' : 'translate-x-0'}`} />
                </button>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-24">
                ...
                <button onClick={() => { setTypeFilter('All'); setAdults(2); setChildren(0); setRooms(1); }}
                  className="text-sm font-bold text-white px-6 py-3 rounded-xl" style={{ background: '#16a34a' }}>
                  Clear Filters
                </button>
              </div>
            ) : (
                  ...
                  <Link key={p.id} href={`/stay/rooms/${p.id}${checkIn ? `?checkIn=${checkIn}&checkOut=${checkOut}&guests=${guestFilter}` : ''}`}
                    ...
                    <div className="relative">
                      ...
                      <button onClick={e => toggleWishlist(e, p.id)}
                        ...
                        <Heart className="w-4 h-4 transition-colors" style={{ color: '#16a34a' }} fill={wishlistIds.has(p.id) ? '#16a34a' : 'none'} />
                      </button>
                      {nights > 0 && (
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                        ...
                        {isMultiMode && (() => {
                          ...
                          return inCart ? (
                            <button onClick={e => { e.preventDefault(); e.stopPropagation(); setCart(prev => { const n = prev.filter(c => c.property.id !== p.id); sessionStorage.setItem('roomCart', JSON.stringify(n)); return n; }); }}
                            ...
                            <button onClick={e => { e.preventDefault(); e.stopPropagation(); setCart(prev => { const n = [...prev, { property: p, qty: 1 }]; sessionStorage.setItem('roomCart', JSON.stringify(n)); return n; }); }}
      ...
      {isMultiMode && cart.length > 0 && (() => {
        ...
        return (
          <button onClick={() => { sessionStorage.setItem('roomCart', JSON.stringify(cart)); sessionStorage.setItem('roomCartMeta', JSON.stringify({ checkIn, checkOut, guests: adults + children })); window.location.href = '/stay/book/cart'; }}
            className="md:hidden fixed bottom-6 right-4 z-[60] flex items-center gap-2 text-white rounded-full shadow-2xl px-4 py-3 font-bold text-sm"
            style={{ background: '#16a34a' }}>
            <ShoppingCart className="w-5 h-5" /><span>{totalRooms}</span>
          </button>
        );
      })()}
      ...
      {isMultiMode && cart.length > 0 && (() => {
        const totalRooms = cart.reduce((s, c) => s + c.qty, 0);
        const n = nights;
        const totalPrice = cart.reduce((s, c) => s + Number(c.property.nightly_rate || 0) * (n || 1) * c.qty, 0);
        return (
            ...
            <div className="pointer-events-auto flex items-center justify-between gap-4 bg-gray-900 text-white rounded-2xl shadow-2xl px-5 py-3.5 w-full max-w-md">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {n > 0 && <p className="text-xs text-white/60">KSh {totalPrice.toLocaleString()} · {n} night{n !== 1 ? 's' : ''}</p>}
                </div>
              </div>
              <button onClick={() => { sessionStorage.setItem('roomCart', JSON.stringify(cart)); sessionStorage.setItem('roomCartMeta', JSON.stringify({ checkIn, checkOut, guests: adults + children })); window.location.href = '/stay/book/cart'; }}
      ...
      {showDate && (
        <DatePickerModal checkIn={checkIn} checkOut={checkOut}
          onConfirm={(ci, co) => { setCheckIn(ci); setCheckOut(co); setShowDate(false); }}
          onClose={() => setShowDate(false)} />
      )}
      {showGuests && (
        <GuestsModal adults={adults} children={children} rooms={rooms}
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end md:hidden" onClick={() => setShowSortSheet(false)}>
          ...
          <div className="relative bg-white rounded-t-2xl shadow-2xl" style={{ paddingBottom: 'calc(2rem + env(safe-area-inset-bottom))' }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 pt-5 pb-3">
              ...
              <button onClick={() => setShowSortSheet(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center"><X className="w-4 h-4 text-gray-600" /></button>
            </div>
            {([['price_asc','Price (lowest first)'],['price_desc','Price (highest first)'],['name','Name A–Z']] as [string, string][]).map(([v, l]) => (
              <label key={v} className="flex items-center gap-4 px-5 py-3.5 border-b border-gray-100 cursor-pointer">
                <input type="radio" name="sort_sheet" checked={sortBy === v} onChange={() => { setSortBy(v as 'price_asc'|'price_desc'|'name'); setShowSortSheet(false); }} className="w-5 h-5 accent-blue-600 flex-shrink-0" />
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end md:hidden" onClick={() => setShowFilterSheet(false)}>
          ...
          <div className="relative bg-white rounded-t-2xl shadow-2xl flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
              <button onClick={() => setShowFilterSheet(false)} className="text-blue-600 p-1"><X className="w-5 h-5" /></button>
              <h3 className="text-base font-black text-gray-900">Filters</h3>
              <button onClick={() => { setTypeFilter('All'); setPriceMin(''); setPriceMax(''); }} className="text-sm text-gray-400 font-semibold">Clear</button>
            </div>
            <div className="overflow-y-auto flex-1 px-5 py-4" style={{ overscrollBehavior: 'contain' }}>
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-500 mb-3">KSh {priceMin !== '' ? Number(priceMin).toLocaleString() : '0'} — KSh {priceMax !== '' ? Number(priceMax).toLocaleString() : 'Any'}</p>
                <div className="flex gap-3">
                  <div className="flex-1">
                    <label className="text-xs text-gray-400 mb-1 block">Min (KSh)</label>
                    <input type="number" placeholder="0" value={priceMin} onChange={e => setPriceMin(e.target.value === '' ? '' : Number(e.target.value))} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-blue-500" />
                  </div>
                  <div className="flex-1">
                    <label className="text-xs text-gray-400 mb-1 block">Max (KSh)</label>
                    <input type="number" placeholder="Any" value={priceMax} onChange={e => setPriceMax(e.target.value === '' ? '' : Number(e.target.value))} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-blue-500" />
                  </div>
                </div>
              </div>
              <div>
                ...
                {ROOM_TYPES.filter(t => t !== 'All').map(t => (
                  <label key={t} className="flex items-center gap-4 py-3 border-b border-gray-100 cursor-pointer">
                    <input type="checkbox" checked={typeFilter === t} onChange={() => setTypeFilter(typeFilter === t ? 'All' : t)} className="w-5 h-5 rounded accent-blue-600 flex-shrink-0" />
            ...
            <div className="px-5 pt-4 border-t border-gray-100" style={{ paddingBottom: 'calc(1rem + env(safe-area-inset-bottom))' }}>
              <button onClick={() => setShowFilterSheet(false)}
                className="w-full py-3.5 rounded-xl text-base font-bold text-white" style={{ background: '#16a34a' }}>
      ...
      {showMapView && (
        <div className="fixed inset-0 z-50 flex flex-col md:hidden bg-white">
          <div className="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100 shadow-sm">
            <button onClick={() => setShowMapView(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex-1 relative" style={{ overscrollBehavior: 'contain' }}>
            <iframe src="https://maps.google.com/maps?q=Kogelo,Siaya,Kenya&t=&z=13&ie=UTF8&iwloc=&output=embed"
              className="w-full h-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
            <button onClick={() => { setShowMapView(false); setShowFilterSheet(true); }}
